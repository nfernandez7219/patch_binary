...
int main(int argc, char **argv)
{
        ...
        for (s = 0, iter32 = (uint32_t *)binmap; s < (bin_size - 8); s+=4, iter32++) {
                if ((iter32[0] == SIGNATURE1) && (iter32[1] == SIGNATURE2)) {
                        char *tmp = (char *)iter32;
                        char *crc_end = ((char *)iter32) + bin_size;

                        printf("found signature! patching binary...\n");

                        /* we get checksum of the binary, but entirely skipping the header */
                        checksum = crc32(checksum, binmap, (size_t)(tmp - binmap));
                        tmp = tmp + sizeof(struct binary_header); /* skip the header */
                        checksum = crc32(checksum, tmp, (size_t)(crc_end - tmp));
                        patch_binary((struct binary_header *)iter32, bin_size, checksum);

                        /* now that we have patched the binary in-memory, commit changes to disk. */
                        fseek(bin_file, 0, SEEK_SET);
                        fwrite(binmap, 1, bin_size, bin_file);
                        break;
                }
        }
